...
async def create_notebook(notebook_text: str, notebook_completed: Optional[bool] = False):
    return await create(notebook_text, notebook_completed)


@router.put("/{notebook_id}", response_model=NotebookUpdate, tags=["notebooks"])
async def update_notebook(notebook_id: int, notebook_text: Optional[str], notebook_completed: Optional[bool]):
